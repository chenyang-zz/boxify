...
release-undo-version: ## 撤销最近一次版本号修改（按目标版本批量删除更高版本的 Release/Tag，回退 package.json，清空 RELEASE_NOTES.md，移除 CHANGELOG 更高版本条目）
	...
	TARGET_COMMIT=""; \
	SEEN_TARGET=0; \
	TARGET_MAJOR=$$(echo "$$VERSION_TO_UNDO" | cut -d. -f1); \
	TARGET_MINOR=$$(echo "$$VERSION_TO_UNDO" | cut -d. -f2); \
	TARGET_PATCH=$$(echo "$$VERSION_TO_UNDO" | cut -d. -f3); \
	BEST_VERSION=""; \
	BEST_MAJOR=0; \
	BEST_MINOR=0; \
	BEST_PATCH=0; \
	for COMMIT in $$(git log --format=%H -- frontend/package.json); do \
		...
		CANDIDATE_PATCH=$$(echo "$$CANDIDATE_VERSION" | cut -d. -f3); \
		IS_CANDIDATE_OLDER=0; \
		if [ "$$CANDIDATE_MAJOR" -lt "$$TARGET_MAJOR" ] 2>/dev/null; then \
			IS_CANDIDATE_OLDER=1; \
		elif [ "$$CANDIDATE_MAJOR" -eq "$$TARGET_MAJOR" ] 2>/dev/null; then \
			if [ "$$CANDIDATE_MINOR" -lt "$$TARGET_MINOR" ] 2>/dev/null; then \
				IS_CANDIDATE_OLDER=1; \
			elif [ "$$CANDIDATE_MINOR" -eq "$$TARGET_MINOR" ] 2>/dev/null; then \
				if [ "$$CANDIDATE_PATCH" -lt "$$TARGET_PATCH" ] 2>/dev/null; then \
					IS_CANDIDATE_OLDER=1; \
				fi; \
			fi; \
		fi; \
		if [ "$$IS_CANDIDATE_OLDER" -eq 1 ]; then \
			if [ -z "$$BEST_VERSION" ]; then \
				...
				IS_CANDIDATE_NEWER_THAN_BEST=0; \
				if [ "$$CANDIDATE_MAJOR" -gt "$$BEST_MAJOR" ] 2>/dev/null; then \
					IS_CANDIDATE_NEWER_THAN_BEST=1; \
				elif [ "$$CANDIDATE_MAJOR" -eq "$$BEST_MAJOR" ] 2>/dev/null; then \
					if [ "$$CANDIDATE_MINOR" -gt "$$BEST_MINOR" ] 2>/dev/null; then \
						IS_CANDIDATE_NEWER_THAN_BEST=1; \
					elif [ "$$CANDIDATE_MINOR" -eq "$$BEST_MINOR" ] 2>/dev/null; then \
						if [ "$$CANDIDATE_PATCH" -gt "$$BEST_PATCH" ] 2>/dev/null; then \
							IS_CANDIDATE_NEWER_THAN_BEST=1; \
						fi; \
					fi; \
				fi; \
				if [ "$$IS_CANDIDATE_NEWER_THAN_BEST" -eq 1 ]; then \
					BEST_VERSION="$$CANDIDATE_VERSION"; \
					BEST_MAJOR=$$CANDIDATE_MAJOR; \
					BEST_MINOR=$$CANDIDATE_MINOR; \
					BEST_PATCH=$$CANDIDATE_PATCH; \
