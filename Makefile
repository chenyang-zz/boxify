...
release-undo-version: ## 撤销最近一次版本号修改（支持自动删除远程 Release，回退 package.json，清空 RELEASE_NOTES.md，移除 CHANGELOG 条目）
	@command -v node >/dev/null 2>&1 || (echo "$(COLOR_YELLOW)未安装 Node.js$(COLOR_RESET)" && exit 1); \
	...
	if [ "$$VERSION_TO_UNDO" = "$$REMOTE_VERSION" ] && [ -n "$$REMOTE_VERSION" ]; then \
		echo "$(COLOR_BLUE)正在删除远程 Release: v$$REMOTE_VERSION$(COLOR_RESET)"; \
		gh release delete "v$$REMOTE_VERSION" --yes >/dev/null || (echo "$(COLOR_YELLOW)删除远程 Release 失败: v$$REMOTE_VERSION$(COLOR_RESET)" && exit 1); \
		echo "$(COLOR_GREEN)已删除远程 Release: v$$REMOTE_VERSION$(COLOR_RESET)"; \
	fi; \
