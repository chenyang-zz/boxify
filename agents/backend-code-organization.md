...
1. 对于存在明确状态、依赖或行为聚合的能力，优先使用“结构体 + 方法”实现，而不是散落的包级函数；
2. 结构体应通过构造函数注入依赖（例如 logger、detector、manager），避免在方法中临时创建核心依赖；
3. `service` 层优先持有 core 结构体实例，并通过方法调用完成能力编排。
4. 需要日志的管理类/协调类结构体应显式声明 `logger` 字段（如 `*slog.Logger`），禁止在各方法内重复获取全局 logger；
5. 构造函数接收 logger 参数时，需处理 `nil` 回退（如回退 `slog.Default()`），避免空指针；
6. 建议在构造函数统一添加模块维度字段（如 `module`），保证同类日志可检索与聚合。

### 9.2 注释要求

1. 所有导出类型、导出方法、导出函数必须有 Go 风格注释（注释前缀与符号名一致）；
2. 承担关键业务语义的非导出函数（例如平台分支、解析/转换、边界处理）也必须补充简洁注释；
3. 注释应描述“职责与边界”，避免无信息量注释（例如“设置值到变量”）。
4. 结构体字段注释统一写在字段右侧（行尾注释），避免字段上方连续堆叠注释，提升扫描效率；
5. 当新增或修改结构体时，业务关键字段必须补齐字段级注释，说明字段语义与用途；
6. 后端注释统一使用中文表达，允许保留必要技术名词、协议名与文件名（如 `JSON`、`OpenClaw`、`plugin.json`）；
7. 函数内流程注释与边界说明同样遵循中文优先，避免混用中英文叙述风格。

### 9.3 日志要求

1. 跨平台分支、降级路径、可恢复错误（如目录跳过）需要有 `Warn` 或 `Debug` 日志；
2. 关键流程应记录开始/完成日志，并包含关键字段（如数量、路径、解析结果）；
3. 不在日志中输出敏感信息（密码、密钥、连接串中的凭据）。
4. 外部依赖调用失败（网络请求、文件读写、子进程执行）应记录失败日志，并附带上下文键值（如 `plugin_id`、`path`、`url`、`error`）；
5. 发生回退/兜底（如镜像失败回退官方源、本地缓存回退）时必须打印明确日志，便于排查路径；
6. 正常高频路径优先使用 `Debug`，关键里程碑（初始化完成、安装完成、更新完成）使用 `Info`，避免噪音日志。
